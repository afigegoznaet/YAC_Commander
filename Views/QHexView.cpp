...
QHexView::~QHexView() { delete m_pdata; }
...
void QHexView::showFromOffset(unsigned long long offset) {
...
void QHexView::clear() { verticalScrollBar()->setValue(0); }
...
void QHexView::paintEvent(QPaintEvent *event) {
	...
	QByteArray data =
		m_pdata->getData(firstLineIdx * BYTES_PER_LINE,
						 (lastLineIdx - firstLineIdx) * BYTES_PER_LINE);

	for (uint lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx;
		 lineIdx += 1, yPos += m_charHeight) {
		QString address = QString("%1").arg(lineIdx * 16, 10, 16, QChar('0'));
		painter.drawText(m_posAddr, yPos, address);

		for (uint xPos = m_posHex, i = 0;
			 i < BYTES_PER_LINE
			 && ((lineIdx - firstLineIdx) * BYTES_PER_LINE + i)
			...
			QString val = QString::number(
				(data.at((lineIdx - firstLineIdx) * BYTES_PER_LINE + i) & 0xF0)
					>> 4,
				16);
			...
			val = QString::number(
				(data.at((lineIdx - firstLineIdx) * BYTES_PER_LINE + i) & 0xF),
				16);
		...
		for (uint xPosAscii = m_posAscii, i = 0;
			 ((lineIdx - firstLineIdx) * BYTES_PER_LINE + i)
				 < static_cast<uint>(data.size())
			 && (i < BYTES_PER_LINE);
...
void QHexView::keyPressEvent(QKeyEvent *event) {
	...
	if (event->matches(QKeySequence::MoveToNextPage)) {
		setCursorPos(m_cursorPos
					 + (viewport()->height() / m_charHeight - 1) * 2
						   * BYTES_PER_LINE);
		resetSelection(m_cursorPos);
		setVisible = true;
	}
	...
	if (event->matches(QKeySequence::Copy)) {
		if (m_pdata) {
			...
			QByteArray data = m_pdata->getData(
				m_selectBegin / 2, (m_selectEnd - m_selectBegin) / 2 + 1);
			if (m_selectBegin % 2) {
...
unsigned long long QHexView::cursorPos(const QPoint &position) {
	int pos = -1;

	if ((static_cast<unsigned long long>(position.x()) >= m_posHex)
		&& (static_cast<unsigned long long>(position.x())
			< (m_posHex + HEXCHARS_IN_LINE * m_charWidth))) {
